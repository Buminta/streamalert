...
class TestPagerDutyOutputV2(object):
    ...
    def test_get_default_properties(self):
        """Get Default Properties - PagerDutyOutputV2"""
        props = self.__dispatcher._get_default_properties()
        assert_equal(len(props), 1)
        assert_equal(props['url'],
...
class TestPagerDutyIncidentOutput(object):
    ...
    @mock_s3
    @mock_kms
    def test_dispatch_success(self, get_mock, post_mock, log_info_mock):
        """PagerDutyIncidentOutput dispatch success"""
        alert = self._setup_dispatch()

        # _check_user_exists, _check_service_exists
    ...
    def test_dispatch_failure_bad_everything(self, get_mock, post_mock, log_error_mock):
        """PagerDutyIncidentOutput dispatch failure - Bad User, Bad Policy, Bad Service"""
        alert = self._setup_dispatch()
        # _check_user_exists, _check_policy_exists, _check_service_exists
        get_mock.return_value.status_code.side_effect = [400, 400, 400]
        json_empty = json.loads('{}')
        get_mock.return_value.json.return_value.side_effect = [json_empty, json_empty, json_empty]
